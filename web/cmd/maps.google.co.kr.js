function url2cmd(url, googlemapsUrl) {
  var maps_google_co_jp_maps = url.match(/^https?:\/\/maps\.google\.co\.kr\/maps[\?\/](.+)/);
  var maps_google_co_kr = url.match(/^https?:\/\/maps\.google\.co\.kr\/\?(.+)/);

  if (maps_google_co_kr_maps || maps_google_co_kr) {
    var id = RegExp.$1;
...
function main(id, googlemapsUrl) {
  ...
  var html = '<iframe marginwidth="0" marginheight="0" hspace="0" vspace="0" frameborder="0" scrolling="no" bordercolor="#000000"'
           ...
           + '&t=' + param.t
           + '&q=' + param.q
           + '" name="sample" height="350">이 부분은 온라인 프레임을 사용하고 있습니다.</iframe>';
